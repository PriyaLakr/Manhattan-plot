...
def prepare_file(file,chromosome,pval,position):
    ...
    for chro,posi in position.groupby('chr'):
        ...
        add+=posi['pos'].max() # maximum position within each chromosome
...
def manhattanplot(snpinfile):
    ...
    ax.set_ylim(1,snpinfile["-log10(p)"].max()+5)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--in_file", type=str, help="Name of the csv file containing SNPs/GWAS data to be analysed")
